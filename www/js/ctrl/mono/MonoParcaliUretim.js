function MonoParcaliUretim($scope, srv, $window, $rootScope) 
{
        ...
        return new Promise(async resolve => 
        {
            let TmpQuery = 
            {
                ...
                query : "SELECT " +
                        ...
                        "(SELECT URt_OpKod FROM URUN_ROTALARI where URt_RotaUrunKodu = sip_stok_kod) AS TMPOPNO, " +
                        "(SELECT URt_IsmerkeziveyaGrupKod FROM URUN_ROTALARI where URt_RotaUrunKodu = sip_stok_kod) AS TMPISMERKEZI ," +
                        "(SELECT URt_DegiskenOperasyonSuresi FROM URUN_ROTALARI where URt_RotaUrunKodu = sip_stok_kod) AS TMPSURESI " +
    ...
    function InsertOperasyonKapama(pDrROTAREC,pDrISEMRI,pDrKODU,pDrSAFHANO,pDrOPERASYONKODU,pDrISMERKEZI,pDrMIKTAR,pDrSURE,pSeri,pSira)
    {
        ...
        return new Promise(async resolve => 
        {
            ...
            let TmpResult = await srv.Execute($scope.Firma,'ParcaliOperasyonHareketInsert',TmpInsertData);
    ...
    $scope.BtnEkle = async function()
    {
        ...
        for (let i = 0; i < TmpDrUret.length; i++) 
        {
            ...
            TmpData.TMPSURESI = TmpDrUret[i].TMPSURESI;
            TmpData.TMPISMERKEZI = TmpDrUret[i].TMPISMERKEZI;
            TmpData.TMPOPNO = TmpDrUret[i].TMPOPNO;
    ...
    $scope.BtnKaydet = async function()
    {
        ...
        if($scope.BteIsEmri.txt == "" || document.getElementById("Tarih").value == "" )
        {
            swal("Dikkat", "Lütfen İş emri ve tarih seçmeden geçmeyin.",icon="warning");
            return;
        }
        ...
        for (let i = 0; i < TmpDrUret.length; i++) 
        {
            ...
            await InsertOperasyonKapama(0,$scope.BteIsEmri.txt,TmpDrUret[i].KODU,1,TmpDrUret[i].TMPOPNO,TmpDrUret[i].TMPISMERKEZI,TmpDrUret[i].MIKTAR,TmpDrUret[i].TMPSURESI,$scope.OpSeri,$scope.OpSira)
