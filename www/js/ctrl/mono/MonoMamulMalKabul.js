function MonoMamulMalKabul($scope,srv, $rootScope)
{
    ...
    async function Ekle(pBarkod,pParti,pLot)
    {
        ...
        for(let i = 0;i < $scope.Data.UMP.length;i++)
        {
            ...
            if($rootScope.GeneralParamList.MamulMalKabulDepo != "")
                ...
                if(TmpData.URETTUKET == 1)
                {
                    ...
                    let TmpResult = await srv.Execute($scope.Firma,'SeriNoInsert',TmpSeriData);
                }
    ...
    $scope.BtnBarkodBas = async function()
    {
        ...
        await Ekle($scope.SeriBarkod);
        await EtiketInsert($scope.SeriBarkod);
