function MonoMalKabul($scope,srv, $rootScope)
{
    ...
    function InitObj()
    {
        ...
        $scope.BteFasoncu = 
        {
            ...
            datasource : 
            {
                ...
                query : "select sip_evrakno_seri AS SERI , sip_evrakno_sira AS SIRA , sip_musteri_kod AS CARI, " +
                        ...
                        "from SIPARISLER WHERE sip_tip = 1 and sip_kapat_fl = 0 and sip_stok_kod = '"+$scope.Test+"' " +
        ...
        $scope.BteParti = 
        {
            ...
            {                                
                ...
                {
                    swal("Dikkat", "Lütfen Stok Seçiniz",icon="warning");
                    pCallback(false)
                }
                else
                {
    ...
    {
        var net = new WebTCP('176.236.120.245', 9999);
    ...
    $scope.BtnTartimOnayla = async function()
    {
        ...
        if($rootScope.GeneralParamList.YariMamulGramKontrol == 1)
        {
            ...
            let TmpData = await srv.Execute($scope.Firma,'StokGramDegerGetir',[$scope.BteIsEmri.txt])
    ...
    $scope.UrunKodTanimi = function()
    {
        $scope.Test = $scope.LblStokKodu
        $scope.BteFasoncu = 
        {
            title : "SIPARIS Seçim",
            txt : "",
            ...
            onSelected : async function(pData)
            {
                ...
                {                    
                    ...
                    $scope.SipSeri = pData.SERI
                    $scope.SipSira = pData.SIRA
                    $scope.TxtSpRefMiktar = pData.KALAN
                    $scope.SiparisMiktarGetir()

                }
            }
        }
    }
    $scope.SiparisMiktarGetir = async function()
    {
        let TmpSipQuery = 
                    {
                        db: "{M}." + $scope.Firma,
                        query : "SELECT (sip_miktar - sip_teslim_miktar) AS KALAN FROM  SIPARISLER WHERE sip_evrakno_seri = @sip_evrakno_seri AND sip_evrakno_sira = @sip_evrakno_sira and sip_stok_kod = @sip_stok_kod and sip_tip = 1  ",
                        param : ['sip_evrakno_seri:string|25','sip_evrakno_sira:int','sip_stok_kod:string|50'],
                        value : [$scope.SipSeri,$scope.SipSira,$scope.BteIsEmri.txt]
                    }
                    let TmpResults = await srv.Execute(TmpSipQuery)
                    $scope.Miktar = TmpResults[0].KALAN
    }
}