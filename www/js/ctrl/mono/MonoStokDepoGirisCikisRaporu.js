    ...
    {        
        ...
        if($scope.SaatDurum == true)
        {
            let TmpQuery = 
            {
                ...
                "  FROM STOK_HAREKETLERI WHERE sth_tip IN(0,1,2) AND  sth_tarih  >= @tarih and sth_tarih <= @sontarih and ((sth_giris_depo_no = @depo) or(sth_cikis_depo_no = @depo)) OR (@depo = 0)" +
                " AND (SELECT top 1 sto_cins FROM STOKLAR WHERE sto_kod = sth_stok_kod) = @CINS " +
                "  GROUP BY  " +
                "  sth_stok_kod,sth_tip,sth_cari_kodu,sth_evraktip, " +
                "  sth_giris_depo_no,sth_cikis_depo_no,sth_cins " ,
                param : ['tarih:dateclock','sontarih:dateclock','depo:int','CINS:int'],
                value : [$scope.BaslangicData,$scope.BitisData,$scope.DepoNo,$scope.StokTip]
            }
        ...
        else
        {
            let TmpQuery = 
            {
                ...
                "  sth_stok_kod,sth_tip,sth_cari_kodu,sth_evraktip, " +
                "  sth_giris_depo_no,sth_cikis_depo_no,sth_cins " ,
                param : ['tarih:date','sontarih:date','depo:int','CINS:int'],
                value : [$scope.BaslangicDataDate[0],$scope.BitisDataDate[0],$scope.DepoNo,$scope.StokTip]
            }
            console.log(TmpQuery)
            let TmpData = await srv.Execute(TmpQuery)
            $scope.DataList = TmpData
